 ...
 */
template <typename T>
inline bool esti_plane(Eigen::Matrix<T, 4, 1> &pca_result, const PointVector &points, const T &threshold = 0.1f) {
    if (points.size() < options::MIN_NUM_MATCH_POINTS) {
        return false;
    }

    Eigen::Matrix<T, 3, 1> normvec;

    if (points.size() == options::NUM_MATCH_POINTS) {
